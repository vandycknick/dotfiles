...
function _hydro_prompt --on-event fish_prompt
    ...
    fish --private --command "
        ...
        test -z \"\$$_hydro_git\" && set --universal $_hydro_git \"$blue($red\$branch$blue)$normal \"

        command git diff-index --quiet HEAD 2>/dev/null
        test \$status -eq 1 ||
            count (command git ls-files --others --exclude-standard (command git rev-parse --show-toplevel)) >/dev/null && set info \"$yellow$hydro_symbol_git_dirty$normal\"
        ...
        for fetch in $hydro_fetch false
            ...
            set --universal $_hydro_git \"$blue($red\$branch$blue)$normal \$info\$upstream \"
